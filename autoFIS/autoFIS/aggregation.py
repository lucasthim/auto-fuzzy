...
class Aggregation:
    # def __init__(self, list_premises_ux_by_class, cbin, info):
    def __init__(self, list_premises_ux_by_class, cbin):
        """
        :param list_premises_ux_by_class:
                [[premisas_c1 uC1], [premisas_c2 uC2], ....]
        """
        self.lista_premisas = list_premises_ux_by_class
        self.cbin = cbin
        # self.info = info
        self.dict_aggregation = {"max": self.max_method,
                                 "LinearModel": self.linear,
                                 "LogRegression": self.logregression,
                                 "MQR": self.mqr,
                                 "intMQR": self.int_mqr}

    def aggregation(self, method='MQR', tnorm_int_mqr='min'):
        data_to_aggregate = [self.lista_premisas, self.cbin]
        if method == "intMQR":
            data_to_aggregate += [2, tnorm_int_mqr]

        # Choose a method and return the estimation and summary of model
        return self.dict_aggregation[method](data_to_aggregate)
    ...
    #     i_fold = self.info[1]
    #     blb_i = self.info[0]
    #     print(i_fold+'-'+blb_i+' '+class_name+' finished')
...
def main():
    ...
    libom = [[p1, a1], [p2, a2]]
    output = Aggregation(libom, cbin)

    print (output.aggregation('linear'))
    print (output.aggregation('LogRegression'))
    print (output.aggregation('MQR'))
    print (output.aggregation('intMQR', 'min'))
    print (output.aggregation('max'))
    print (5)
