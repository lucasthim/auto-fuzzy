...
def relative2absolute(list_indexes_premises, conversor):
    ...
    for relative_indexes_premise in list_indexes_premises:
        absolute_indexes_premise = tuple(conversor[j] for j in relative_indexes_premise)
...
def data_blb(ux_train_blb, y_bin_blb):
    ...
    class_num = 0
    size_resample_ux_train = np.floor(pow(size_ux_train_blb, p_r.percent_resample)).astype(int)
    index_ux_train_blb = range(size_ux_train_blb)
    while class_num == 0:
        index_resample = resample(index_ux_train_blb, replace=False, n_samples=size_resample_ux_train)
        ...
        if len(classes_blb) == len(classes):
            class_num = 1
            index_oob = np.array(list(set(list(index_ux_train_blb)) - set(list(index_resample))))
...
def create_data(ref_attributes, size_attributes, premises_by_attribute, premises_contain_negation, ux_train, y_bin):
    ...
    new_premises_by_attribute = gather_numbers_by_sizes(aux, sub_sizes_attributes)
...
def lecture_fuz_one_cv(zipFilePath, file_train, file_test, parameters, cv):
    # Parameters
    ...
    sizes_attributes = fuzzifier.num_prem_by_attribute  # [3, 2, 3]
    premises_by_attribute = fuzzifier.premises_attributes  # [(0,1,2),(3,4),(5,6,7)]
    ...
    attributes_information = [sizes_attributes, premises_by_attribute, ref_attributes, premises_contain_negation]
...
def inference_fuzzy(data_fuz, parameters, info, ensemble="BagFIS"):  # Formulation, Association, Aggregation, Decisions
    ...
    try:
        ...
        arbol = form.gen_ARB(max_size_premises, t_norm, par_area, par_over, par_pcd)
        ...
        premises_weights_names = output_aggregation[0]  # 0: [[premises],weights,method_name]
...
def classifiers_aggregation(indexes_premises_by_class, y_bin, aggregation_method, freq_classes, info):
    ...
    try:
        ...
        premises_weights_names = output_aggregation[0]  # 0: premises, 1: weights
        ...
        if sum(status) != number_classes:
            ...
            f5 = Decisions(estimation_classes, freq_classes)
            train_bin_prediction = f5.dec_max_pert()
            outputs = [output_aggregation[0], train_bin_prediction, estimation_classes]
    except ValueError as e:
        print (e)
        # report = e  # .append("\n" + str(e))
        successful = 0
...
def create_csv_summaries(root):
    ...
    file_res.write("Dataset, Accuracy, Std(Ac), Num_Regras, Successful_Classifiers, Time(s)\n")
    file_res.close()
    return csv_summary
